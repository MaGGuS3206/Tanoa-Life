...
switch (_code) do {
            ...
            if (vehicle player != player && alive vehicle player) then {
                ...
                if (!(isNil "_list")) then {
                    ...
                    if ((KINDOF_ARRAY(cursorObject,_list)) && {player distance cursorObject < 7} && {isNull objectParent player} && {alive cursorObject} && {!life_action_inUse}) then {
						...
						if ((cursorTarget isKindOf "Land_Wreck_Traw_F" OR cursorTarget isKindOf "Land_Wreck_Traw2_F")&& player distance cursorTarget < 10 && vehicle player == player && alive cursorTarget) then {
    ...
    case 38: {
        ...
        if (_shift && playerSide in [west,independent]) then {
            if (vehicle player != player && (typeOf vehicle player) in ["C_Offroad_01_F","B_MRAP_01_F","C_SUV_01_F","C_Hatchback_01_sport_F","B_Heli_Light_01_F","B_Heli_Transport_01_F","B_GEN_Offroad_01_gen_F","C_Offroad_02_unarmed_F","C_Hatchback_01_F","B_LSV_01_unarmed_F","B_CTRG_LSV_01_light_F"]) then {
