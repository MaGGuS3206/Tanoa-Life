/*File: fn_profSetUp.sqfAuthor: Jacob "PapaBear" Tyler
...
disableSerialization;
 ...
 {
 	_numSpaces = 0;
 	_profText = [_x select 0] call life_fnc_profType;_charCount = count _profText;
 	_numSpaces = _spacesTill - _charCount;
 	_data = missionNamespace getVariable (_x select 0);
 	_profLevel = _data select 0;_profExp = _data select 1;
 	_nextLevel = 0;
 	_nextLevel = 4 * ( _profLevel^3 ) ;_nextLevel2 =  6 * ( _profLevel^2 ) ;
